...
if args.resume is None:
    if args.test_only:
        raise RuntimeError("No model specified for testing")
    checkpoint = None
else:
...
args.output_folder = join(
    "runs", args.exp_name, start_time.strftime("%Y-%m-%d_%H-%M-%S")
)
...
if args.resume is None and args.mode == "netvlad":
    args.cluster_ds = datasets_ws.BaseDataset(
        args, args.datasets_folder, "pitts30k", "train"
    )
...
if checkpoint is None:
    ...
    last_epoch_num, recalls, best_r5, not_improved_num = util.resume_from_state(
        checkpoint, model, optimizer, scheduler
    )
    if recalls[1] > best_r5:
        best_r5 = recalls[1]
    logging.debug(
        f"Successfully loaded model from checkpoint (epoch: {last_epoch_num}, recalls: {recalls})"
    )
del checkpoint
...
if not args.test_only:
    ...
    for epoch_num in range(last_epoch_num + 1, args.epochs_num + 1):
            ...
            for images, triplets_local_indexes, _ in tqdm(triplets_dl, ncols=100):
                ...
                for triplets in triplets_local_indexes:
                    ...
                    if criterion_sos is not None:
                        loss += args.sos_lambda * criterion_sos(
                            features[queries_indexes],
                            features[positives_indexes],
                            features[negatives_indexes],
                        )
        ...
        util.save_checkpoint(
            args, state, is_best, filename=f"model_{epoch_num:02d}.pth"
        )
    ...
    logging.info(
        f"Trained for {epoch_num:02d} epochs, in total in {str(datetime.now() - start_time)[:-7]}"
    )
